	...
	for _, bd := range d.Bags {
		...
		if len(bd.Keys) > 0 {
			...
			for key, kindName := range bd.Keys {
				...
				// The declared SPELLING, not a folded one. Three separate
				// arguments, and the third is the one that closes the question.
				//
				// (1) Correctness. A VARIANT key is case-sensitive here, so the
				// folded name may not exist: measured on logs.k8s_logs,
				// kv['tableID'] is present on 282,718 rows and kv['tableid'] on
				// 0. Folding made every consumer that rebuilds the subscript ask
				// about a key that is not there, and the bag-statistics probe did
				// exactly that — a descriptor correctly declaring `tableID`,
				// generated by this tool, probed kv['tableid'], found nothing,
				// and reported the key as vanished.
				//
				// (2) Scale. This is not a corner. Over the settled hour
				// [2026-08-20 02:00, 03:00), the bag holds 78 distinct keys and
				// 26 of them carry uppercase — a third — and the single busiest
				// key in the hour is `tableID` itself. The names are ordinary Go
				// logger output: controllerGroup, controllerKind, reconcileID,
				// TiFlash, needWait, safePoint, serverID, startKey, indexID.
				//
				// (3) Expressiveness, which forecloses the tempting middle road
				// of folding for LOOKUP while keeping the original for SQL. Two
				// live keys on this table differ only by case:
				//
				//	kv['safepoint']   1,740 rows
				//	kv['safePoint']     459 rows
				//
				// So a folded declaration cannot say which one it means. There
				// exist real tables a folding schema simply cannot describe, and
				// a case-insensitive lookup over them has two right answers —
				// which means it must be an error or a warning, never a silent
				// pick. Lookup is therefore exact too (see Bag.field), and the
				// one place a fold is correct — asking "did a human type this in
				// the wrong case" — reports every candidate rather than choosing
				// (see observedSpellings).
				//
				// This is the boundary that has to stay explicit, because the
				// two halves of the schema disagree on purpose:
				//
				//	FIELD names are folded. `Level:error` and `level:error` are
				//	the same field, because a field name is a label this library
				//	chose and users type it however they like.
				//
				//	BAG KEYS are not. `kv['tableID']` and `kv['tableid']` are
				//	different keys in the data, so folding one into the other
				//	would silently retarget the lookup. The compiler's own
				//	advisory says precisely this to users; the library must not
				//	then do the opposite.
				if _, dup := b.Keys[key]; dup {
					return Schema{}, nil, fmt.Errorf(
						"schema: bag %q declares key %q twice", bd.Column, key)
				}
				b.Keys[key] = kind
			}
		}
