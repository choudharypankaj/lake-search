...
// digest returns the column-list digest the descriptor recorded at probe time,
// if it was generated by `introspect build`. A hand-written descriptor has none,
// and drift is then reported from the column and index lists alone.
func (sf *schemaFlags) digest() string {
	if sf.file == "" {
		return ""
	}
	...
	d, err := databend.ParseDef(raw)
	if err != nil || d.Introspect == nil {
		return ""
	}
	return d.Introspect.ColumnsDigest
}
